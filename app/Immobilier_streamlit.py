...
URL_GITHUB = "https://github.com/Math1720182/Mon-Analyse-Immo-App/releases/download/v2.0.0/dvf_clean_2021_2025_v2.parquet"

def telecharger_fichier_si_absent_dvf():
...
URL_GITHUB = "https://github.com/Math1720182/Mon-Analyse-Immo-App/releases/download/v1.0.0/BPE25.parquet"

def telecharger_fichier_si_absent_BPE():
    
    if not os.path.exists(NOM_FICHIER_LOCAL):
        with st.spinner("Téléchargement initial de la base BPE en cours..."):
            reponse = requests.get(URL_GITHUB, timeout = 30)
...
telecharger_fichier_si_absent_BPE()
...
pages = {
    "**MENU**": [
        st.Page("app/intro.py", title="Introduction"),
        st.Page("app/analyse.py", title="Tableau de bord", default = True),
        st.Page("app/simulateur.py", title="Mon projet immobilier"),
        st.Page("app/stats.py", title="Statistiques"),
    ]
}
