...
def get_zone_sales(code_commune, type_bien, lat_target, lon_target, rayon_m):
        ...
        WITH ventes_commune AS (
            SELECT 
                *,
                2 * 6371000 * ASIN(
                    SQRT(
                        POWER(SIN(RADIANS(latitude - {lat_target}) / 2.0), 2) +
                        COS(RADIANS({lat_target})) * COS(RADIANS(latitude)) *
                        POWER(SIN(RADIANS(longitude - {lon_target}) / 2.0), 2)
                    )
                ) AS distance_m
            FROM 'dvf_clean_2021_2025_v2.parquet'
            WHERE "Code commune" = '{code_commune}'
              AND "Type local" = '{type_bien}'
    ...
    df_zone = connexion.execute(requete_sql).df()
    connexion.close()
...
def geocode_adresse(adresse):
    url = f"https://api-adresse.data.gouv.fr/search/?q={requests.utils.quote(adresse)}&limit=1" #Encodage auto de l'URL via requests.utils.quote
    try:
        ...
        st.error(f"Erreur de géocodage BAN : {e}")
