...
def bpe_df(lat, lon, rayon_degres = extension_distance):
    ...
    SELECT
        *,
        calculer_distance_metres({lat}, {lon}, LATITUDE, LONGITUDE) AS distance
    FROM 'BPE25.parquet'
    WHERE LATITUDE BETWEEN {lat_min} AND {lat_max}
      AND LONGITUDE BETWEEN {lon_min} AND {lon_max}
    """
    
    df_bpe = connexion.execute(requete_sql).df()
    connexion.close()
...
#Pharmacie
pharmacie = bpe_df_return[bpe_df_return['TYPEQU'] == 'D307'].sort_values(by='distance').iloc[0]
nom_pharmacie = pharmacie["NOMRS"]
distance_pharmacie = pharmacie["distance"]

#Dentiste
dentiste = bpe_df_return[bpe_df_return['TYPEQU'] == 'D277'].sort_values(by='distance').iloc[0]
nom_dentiste = dentiste["NOMRS"]
distance_dentiste = dentiste["distance"]

#Kiné
kine = bpe_df_return[bpe_df_return['TYPEQU'] == 'D279'].sort_values(by='distance').iloc[0]
nom_kine = kine["NOMRS"]
distance_kine = kine["distance"]

#Supermarché
supermarche = bpe_df_return[bpe_df_return['TYPEQU'] == 'B105'].sort_values(by='distance').iloc[0]
nom_supermarche = supermarche["NOMRS"]
distance_supermarche = supermarche["distance"]

#Boulangerie
boulangerie = bpe_df_return[bpe_df_return['TYPEQU'] == 'B207'].sort_values(by='distance').iloc[0]
nom_boulangerie = boulangerie["NOMRS"]
distance_boulangerie = boulangerie["distance"]

#Coiffeur
coiffeur = bpe_df_return[bpe_df_return['TYPEQU'] == 'A501'].sort_values(by='distance').iloc[0]
nom_coiffeur = coiffeur["NOMRS"]
distance_coiffeur = coiffeur["distance"]

#La Poste
poste = bpe_df_return[bpe_df_return['TYPEQU'] == 'A206'].sort_values(by='distance').iloc[0]
nom_poste = poste["NOMRS"]
distance_poste = poste["distance"]

#Ecole primaire
ecole = bpe_df_return[bpe_df_return['TYPEQU'] == 'C107'].sort_values(by='distance').iloc[0]
nom_ecole = ecole["NOMRS"]
distance_ecole = ecole["distance"]

#Collège
college = bpe_df_return[bpe_df_return['TYPEQU'] == 'C201'].sort_values(by='distance').iloc[0]
nom_college = college["NOMRS"]
distance_college = college["distance"]

#Lycée
lycee = bpe_df_return[bpe_df_return['TYPEQU'] == 'C301'].sort_values(by='distance').iloc[0]
...
with col3:
    ...
    """,
        ...
        st.markdown("##### Urgence")
        st.markdown(f"###### {distance_affiche} - *{nom_urgence}*")
        ...
        st.markdown(f"###### {distance_affiche} - *{nom_generaliste}*")
        ...
        st.markdown(f"###### {distance_affiche} - *{nom_pharmacie}*")
        ...
        st.markdown(f"###### {distance_affiche} - *{nom_dentiste}*")
        ...
        st.markdown(f"###### {distance_affiche} - *{nom_kine}*")
    ...
    """,
        ...
        st.markdown(f"###### {distance_affiche} - *{nom_supermarche}*")
        ...
        st.markdown(f"###### {distance_affiche} - *{nom_boulangerie}*")
        ...
        st.markdown(f"###### {distance_affiche} - *{nom_coiffeur}*")
        ...
        st.markdown(f"###### {distance_affiche} - *{nom_poste}*")
    ...
    """,
        ...
        st.markdown(f"###### {distance_affiche} - *{nom_ecole}*")
        ...
        st.markdown("##### Collège")
        st.markdown(f"###### {distance_affiche} - *{nom_college}*")
        ...
        st.markdown("##### Lycée")
        st.markdown(f"###### {distance_affiche} - *{nom_lycee}*")
