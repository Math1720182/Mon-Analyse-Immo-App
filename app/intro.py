...
def data_intro():
    ...
    requete_sql = f"""
        SELECT 
            COUNT(*) AS nb_ventes,
            SUM("Valeur fonciere") / 1e9 AS montant_total
        FROM 'dvf_clean_2021_2025_v2.parquet'
    """
    connexion = duckdb.connect()
    data_intro = connexion.execute(requete_sql).df()
    connexion.close()

    return data_intro

data_intro_return = data_intro()

nb_ventes = int(data_intro_return['nb_ventes'].iloc[0])
montant_total = data_intro_return['montant_total'].iloc[0]

st.title('Introduction', text_alignment = 'center')
st.markdown('## Analyse du marché immobilier en France', text_alignment = 'center')
