...
#----Conditions SQL----
cond_annees = f"year BETWEEN {selected_year_range[0]} AND {selected_year_range[1]}"

types_str = str(tuple(types_to_filter)).replace(",)",")")
cond_type = f"\"Type local\" IN {types_str}"

dpe_str = str(tuple(dpe_choisis)).replace(",)", ")")
cond_dpe = f"classe_bilan_dpe IN {dpe_str}"

#Seule chaîne de caractères SQL
clause_where_globale = f"WHERE {cond_annees} AND {cond_type} AND {cond_dpe}"
...
def get_map_data(clause_where):
    ...
    connexion = duckdb.connect()
    df_grouped = connexion.execute(requete_sql).df()
    connexion.close()
    
    return df_grouped

geojson_france = get_geojson()
df_map = get_map_data(clause_where_globale)
...
@st.cache_data
def get_map_data_communes(clause_where=""):
    requete_sql = f"""
        SELECT
            "Nom departement",
            "Code departement",
...
df_map_global = get_map_data_communes(clause_where_globale)
...
def get_communes(clause_where=""):
    ...
    connexion = duckdb.connect()
    df_resultat = connexion.execute(requete_sql).df()
    connexion.close()

    return df_resultat['Commune'].tolist()

@st.cache_data
def get_stats_commune(commune_nom, clause_where=""):
    commune_clean = commune_nom.replace("'", "''")

    if clause_where:
        filtre = f"{clause_where} AND \"Commune\" = '{commune_clean}'"
    else:
        filtre = f"WHERE \"Commune\" = '{commune_clean}'"
    ...
    requete_sql = f"""
        SELECT 
            ...
            MEDIAN(CASE WHEN year = (SELECT MIN(year) FROM 'dvf_clean_2021_2025_v2.parquet' {filtre}) THEN "Price per surface" END) AS first_year_median,

            MEDIAN(CASE WHEN year = (SELECT MAX(year) FROM 'dvf_clean_2021_2025_v2.parquet' {filtre}) THEN "Price per surface" END) AS last_year_median
        FROM 'dvf_clean_2021_2025_v2.parquet'
        {filtre}
    """
    connexion = duckdb.connect()
    df_stats = connexion.execute(requete_sql).df()
    connexion.close()

    return df_stats.iloc[0].to_dict()

commune = get_communes(clause_where_globale)

index_defaut = commune.index('Compiègne (60)') if 'Compiègne (60)' in commune else 0
...
with col1:
    ...
    stats_commune = get_stats_commune(commune_select, clause_where_globale)
...
def global_kpi(clause_where=""):
    ...
    """
    connexion = duckdb.connect()
    global_kpi = connexion.execute(requete_sql).df()
    connexion.close()

    return global_kpi.iloc[0].to_dict()

global_kpi_return = global_kpi(clause_where_globale)
...
@st.cache_data
def global_kpi_frame(clause_where=""):
    requete_sql = f"""
        SELECT
    ...
    """
    connexion = duckdb.connect()
    global_kpi_frame = connexion.execute(requete_sql).df()
    connexion.close()

    return global_kpi_frame

global_kpi_frame_return = global_kpi_frame(clause_where_globale)
...
@st.cache_data
def get_stats_departement(clause_where=""):
    requete_sql = f"""
        SELECT
            "Nom departement",
            COUNT(*) AS nombre_vente,
...
stats_dep = get_stats_departement(clause_where_globale)
...
@st.cache_data
def cache_top10_m2(clause_where=""):
    ...
    requete_sql = f"""
        ...
        """
    connexion = duckdb.connect()
    df_top10 = connexion.execute(requete_sql).df()
    connexion.close()
...
df_top10, top10_m2 = cache_top10_m2(clause_where_globale)
