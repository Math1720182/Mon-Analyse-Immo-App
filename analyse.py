...
if "geojson_france" not in st.session_state:
    with st.spinner("Chargement du fond de carte de la France..."):
        url = "https://raw.githubusercontent.com/gregoiredavid/france-geojson/master/departements-version-simplifiee.geojson"
        response = requests.get(url)
        if response.status_code == 200:
            st.session_state.geojson_france = response.json()
        else:
            st.error("Erreur lors du téléchargement du fond de carte.")
            st.session_state.geojson_france = None

geojson_france = st.session_state.geojson_france

df_clean['year'] = df_clean['Date mutation'].str[:4]
df_map = df_clean[['Code departement', 'Nom departement', 'Price per surface', 'year']]
df_map = df_map.groupby(['Code departement', 'year', 'Nom departement'])['Price per surface'].median().reset_index()
...
#------------------------------------------------------
#--Surface habitable par terrain (maison uniquement)---
#------------------------------------------------------
...
stats_dep = stats_dep.merge(moyenne_par_dep, on="Nom departement", how='left')
...
with col1:
    ...
    @st.fragment #Containeur pour ne pas rafraichir toute la page à chaque changement de valeur
