...
df_filtered = df_clean[
    (df_clean['year'].between(selected_year_range[0], selected_year_range[1])) &
    (df_clean['Type local'].isin(types_to_filter))]
...
@st.cache_data
def get_map_data_communes(df):
    df_grouped = (
        df.groupby(['Nom departement', 'Code departement', 'Code commune', 'year', 'Commune'], observed=True)['Price per surface']
        .median()
        .reset_index()
    )

    df_grouped['Code departement'] = df_grouped['Code departement'].astype(str).str.split('.').str[0]
    return df_grouped


gdf_france_communes = get_geojson_communes()
df_map_global = get_map_data_communes(df_filtered)
