...
def get_map_data(df):
    df_grouped = (
        df.groupby(['Code departement', 'year', 'Nom departement', 'Type local'], observed=True)['Price per surface']
        .median()
        .reset_index()
    )
...
df_map = get_map_data(df_clean[['Code departement','year','Nom departement','Price per surface', 'Type local']])
...
def afficher_section_carte(df_map, geojson_france):
    ...
    types_disponible = ['Maison', 'Appartement']
    
    st.write("Type de bien :")
    
    col1, col2, _ = st.columns([0.15, 0.3, 1])
    cols = [col1, col2]
    
    types_selectionne = []
    
    for i, option in enumerate(types_disponible):
        actif = cols[i].toggle(option, value=True)
        if actif:
            types_selectionne.append(option)
    
    if not types_selectionne:
        st.warning('Veuillez sélectionner au moins une catégorie')
        return

    df_filtered = df_map[df_map['Type local'].isin(types_selectionne)]

    df_map = (
        df_filtered.groupby(['Code departement', 'year', 'Nom departement'], observed=True)['Price per surface']
        .median()
        .reset_index()
    )
...
commune = sorted(df_clean['Commune'].dropna().unique())

index_defaut = commune.index('COMPIEGNE (60)')
...
with col1:
    ...
    nombre_vente_commune = df_clean[df_clean['Commune']==commune_select]['Price per surface'].count()
    st.caption(f"Nombre de vente (sur 5ans): **{nombre_vente_commune}**")

df_commune = df_clean[df_clean['Commune']==commune_select]
...
st.subheader('Analyse globale du marché entre 2021 et 2025')
...
def calculer_stats_departements(df_clean, stats_dep):
    df_maison = df_clean[df_clean['Type local'] == 'Maison']
...
def cache_top10_m2(df_clean):
    top10_m2 = (
        df_clean.groupby("Nom departement", observed=True)["Price per surface"]
        .mean()
        .nlargest(10)
        .index
    )
    
    df_top10 = df_clean[df_clean["Nom departement"].isin(top10_m2)][["Nom departement", "Price per surface"]]
