...
#Prenons donc un échantillon de 100 ventes
...
sample_mean = sample['Valeur fonciere'].mean() #a faire avant de grouper les valeurs
sample_std = sample['Valeur fonciere'].std() #idem

sample_m2 = df_clean.dropna(subset=['Price per surface']).sample(100, random_state=42).copy()
sample_m2['Price per surface'] = pd.to_numeric(sample['Price per surface'], errors = 'coerce')
#---Calculs détaillés-----
# 1)Calcul des fréquences observés
sample = sample['Valeur fonciere'].value_counts().reset_index()
sample.columns = ['Valeur fonciere', 'fo']
sample = sample.sort_values(by = 'Valeur fonciere', ascending = True).reset_index(drop = True)
# 2)Calcul des fréquences relatives
sample['fr'] = sample['fo'] / 100
# 3)Calcul des fréquences cumulées
sample['Fo'] = sample['fr'].cumsum()
# 4)Calcul de la cote Z

sample['Cote Z'] = (sample['Valeur fonciere'] - sample_mean)/sample_std
# 5)Calcul de la p_value et de D
stat, p_value = kstest(sample['Cote Z'], 'norm')

#Conclusion : Ça ne suit pas une loi normal, mais une loi log-normal (si je transforme les prix en log, p_value > 0.05).
#Il ne faut donc plus utiliser la moyenne et l'écart-type mais la médiane et l'écart-interquartile.

#---Calcul KS rapide (supprime l'ensemble des étapes du calcul détaillé)----
...
groupes = []
nb_groupe = groupe_KW_2.groupby('Nombre pieces principales').count().reset_index()
nb_groupe = nb_groupe['Nombre pieces principales'].count() #Nombre de groupe pour calculer le seuil critique avec chi-deux table

for _, group in groupe_KW_2.groupby('Nombre pieces principales')['Price per surface']:
